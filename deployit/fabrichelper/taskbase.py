...
class LetsEncryptCreateCertificate(BaseTask):
    ...
    @calc_duration
    def run(self):
        for server_name in env.server_names:
            sudo('letsencrypt certonly --email={} --agree-tos -a webroot --webroot-path=/tmp/letsencrypt-auto -d {}'.format(env.ssl_email, server_name))
