...
class EnvTask(Task):
    ...
    def dynamic_env(self):
        ...
        if not 'settings_module' in env:
            env.settings_module =  "%s.settings.%s" % (env.project_name, env.env_name)

        if not 'requirements_file' in env:
            env.requirements_file = '%s.txt' % env.env_name
